...
@section('breadcrumb')
<a href="{{ route('admin.dashboard') }}" class="hover:text-primary">Beranda</a> <span>/</span> <a href="{{ route('admin.arsip.index') }}" class="hover:text-primary">Manajemen Arsip</a> <span>/</span> <span class="text-slate-700 font-medium">Registrasi</span>
@endsection
@section('content')
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="no_berkas" value="{{ old('no_berkas') }}" required placeholder="Contoh: 000/7/VI.01/2026" class="w-full px-3.5 py-2.5 rounded-lg border-slate-300 text-sm focus:ring-primary focus:border-primary @error('no_berkas') border-red-400 @enderror">
                        @error('no_berkas')<p class="text-xs text-red-500 mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div class="sm:col-span-2">
                        <label class="block text-xs font-semibold text-slate-600 mb-1.5">Kurun Waktu</label>
                        <input type="text" name="kurun_waktu" value="{{ old('kurun_waktu') }}" placeholder="Contoh: 2026 atau Jan-Jun 2026" class="w-full px-3.5 py-2.5 rounded-lg border-slate-300 text-sm focus:ring-primary focus:border-primary @error('kurun_waktu') border-red-400 @enderror">
                        @error('kurun_waktu')<p class="text-xs text-red-500 mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-600 mb-1.5">Jumlah Hal/Map/Bundle</label>
                        <input type="text" name="jumlah_halaman_bundle" value="{{ old('jumlah_halaman_bundle') }}" placeholder="e.g. 5 hal" class="w-full px-3.5 py-2.5 rounded-lg border-slate-300 text-sm focus:ring-primary focus:border-primary">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-600 mb-1.5">Jumlah Berkas</label>
                        <div class="flex rounded-lg border border-slate-300 overflow-hidden focus-within:ring-1 focus-within:ring-primary focus-within:border-primary">
                            <input type="number" min="1" name="jumlah_berkas" value="{{ old('jumlah_berkas', 1) }}" class="w-full border-0 text-sm focus:ring-0">
                            <span class="flex items-center px-3 text-xs font-medium text-primary bg-primary/10 shrink-0">berkas</span>
                        </div>
                    </div>
                    <div class="sm:col-span-2">
                        <label class="block text-xs font-semibold text-slate-600 mb-1.5">Nomor Item Arsip</label>
                        <input type="text" name="no_item_arsip" value="{{ old('no_item_arsip') }}" placeholder="Masukkan nomor item" class="w-full px-3.5 py-2.5 rounded-lg border-slate-300 text-sm focus:ring-primary focus:border-primary">
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="uraian_arsip" rows="2" placeholder="e.g. Umum" class="w-full px-3.5 py-2.5 rounded-lg border-slate-300 text-sm focus:ring-primary focus:border-primary">{{ old('uraian_arsip') }}</textarea>
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-semibold text-slate-600 mb-1.5">Tanggal Diarsipkan <span class="text-red-500">*</span></label>
                            <input type="date" name="tanggal_diarsipkan" value="{{ old('tanggal_diarsipkan', date('Y-m-d')) }}" required class="w-full px-3.5 py-2.5 rounded-lg border-slate-300 text-sm focus:ring-primary focus:border-primary @error('tanggal_diarsipkan') border-red-400 @enderror">
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <textarea name="tingkat_perkembangan" rows="2" placeholder="Diarsipkan di Unit Pengolah Kearsipan Bappeda" class="w-full px-3.5 py-2.5 rounded-lg border-slate-300 text-sm text-slate-500 focus:ring-primary focus:border-primary">{{ old('tingkat_perkembangan') }}</textarea>
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-600 mb-1.5">Nomor Rak <span class="text-red-500">*</span></label>
                        <input type="text" name="no_rak" value="{{ old('no_rak') }}" placeholder="e.g. 1" class="w-full px-3.5 py-2.5 rounded-lg border-slate-300 text-sm focus:ring-primary focus:border-primary">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-600 mb-1.5">Nomor Boks <span class="text-red-500">*</span></label>
                        <input type="text" name="no_boks" value="{{ old('no_boks') }}" placeholder="BKS-2023-001" class="w-full px-3.5 py-2.5 rounded-lg border-slate-300 text-sm focus:ring-primary focus:border-primary">
                        <p class="text-[0.7rem] text-slate-400 mt-1">Sistem menyarankan boks terakhir yang tersedia.</p>
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-600 mb-1.5">Nomor Folder</label>
                        <input type="text" name="no_folder" value="{{ old('no_folder', 1) }}" class="w-full px-3.5 py-2.5 rounded-lg border-slate-300 text-sm focus:ring-primary focus:border-primary">
                    </div>
                    <input type="hidden" name="lokasi_simpan" value="{{ old('lokasi_simpan') }}">
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-3 gap-2 mb-1">
                        <label class="flex items-center gap-2 text-sm text-slate-600">
                            <input type="radio" name="status_retensi" value="aktif" onchange="document.getElementById('nasibAkhirWrap').classList.add('hidden')" {{ old('status_retensi', 'aktif') === 'aktif' ? 'checked' : '' }} class="rounded text-primary focus:ring-primary">
                            Aktif
                        </label>
                        <label class="flex items-center gap-2 text-sm text-slate-600">
                            <input type="radio" name="status_retensi" value="inaktif" onchange="document.getElementById('nasibAkhirWrap').classList.add('hidden')" {{ old('status_retensi') === 'inaktif' ? 'checked' : '' }} class="rounded text-primary focus:ring-primary">
                            Inaktif
                        </label>
                        <label class="flex items-center gap-2 text-sm text-slate-600">
                            <input type="radio" name="status_retensi" value="inaktif" onchange="document.getElementById('nasibAkhirWrap').classList.remove('hidden')" {{ old('nasib_akhir') ? 'checked' : '' }} class="rounded text-primary focus:ring-primary">
                            Nasib Akhir
                        </label>
                    </div>
                    <div id="nasibAkhirWrap" class="mt-3 {{ old('nasib_akhir') ? '' : 'hidden' }}">
                        <label class="block text-xs font-semibold text-slate-600 mb-1.5">Keterangan Nasib Akhir</label>
                        <input type="text" name="nasib_akhir" value="{{ old('nasib_akhir') }}" placeholder="cth: Musnah/Permanen" class="w-full px-3.5 py-2.5 rounded-lg border-slate-300 text-sm focus:ring-primary focus:border-primary">
                    </div>
