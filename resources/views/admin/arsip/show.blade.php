...
<div class="flex items-start justify-between mb-6 flex-wrap gap-3">
    ...
    <div class="flex items-center gap-2 flex-wrap">
        <a href="{{ route(auth()->user()->isAdmin() ? 'admin.arsip.index' : 'operator.arsip.index') }}" class="inline-flex items-center gap-2 px-3.5 py-2 text-sm font-semibold rounded-lg border border-slate-300 bg-white text-slate-700 hover:bg-slate-50 transition shadow-sm">
        ...
        <a href="{{ route(auth()->user()->isAdmin() ? 'admin.arsip.print-label' : 'operator.arsip.print-label', $arsip) }}" target="_blank" class="inline-flex items-center gap-2 px-3.5 py-2 text-sm font-semibold rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 transition shadow-sm">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 6 2 18 2 18 9"/><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></svg>
            Cetak Label Stiker
        </a>
        <a href="{{ route(auth()->user()->isAdmin() ? 'admin.arsip.edit' : 'operator.arsip.edit', $arsip) }}" class="inline-flex items-center gap-2 px-3.5 py-2 text-sm font-semibold rounded-lg bg-accent-gold text-primary-dark hover:bg-accent-gold-dark transition shadow-sm">
        ...
        <a href="{{ route(auth()->user()->isAdmin() ? 'admin.arsip.download' : 'operator.arsip.download', $arsip) }}" class="inline-flex items-center gap-2 px-3.5 py-2 text-sm font-semibold rounded-lg bg-primary text-white hover:bg-primary-light transition shadow-sm">
    ...
    <div class="space-y-6">
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5 text-center">
            ...
            <div class="mt-4 flex gap-2">
                ...
                <a href="{{ route(auth()->user()->isAdmin() ? 'admin.arsip.download' : 'operator.arsip.download', $arsip) }}" class="flex-1 py-2 text-xs font-semibold rounded-lg bg-primary text-white hover:bg-primary-light transition">Unduh</a>
            </div>
            @endif
        </div>
        ...
        <div class="flex-1 bg-slate-100 p-2 overflow-hidden flex items-center justify-center">
            ...
            @else
                <div class="text-center p-6 bg-white rounded-xl shadow-sm">
                    ...
                    <a href="{{ route(auth()->user()->isAdmin() ? 'admin.arsip.download' : 'operator.arsip.download', $arsip) }}" class="inline-flex items-center gap-2 px-4 py-2 text-xs font-semibold rounded-lg bg-primary text-white">Unduh File</a>
                </div>
            @endif
        </div>
    </div>
