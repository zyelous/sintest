...
<div class="flex items-start justify-between mb-6 flex-wrap gap-3">
    ...
    <div class="flex items-center gap-2">
        <a href="{{ route('admin.arsip.index') }}" class="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-lg border border-slate-300 bg-white text-slate-700 hover:bg-slate-50 transition shadow-sm">
        ...
        <a href="{{ route('admin.arsip.edit', $arsip) }}" class="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-lg bg-accent-gold text-primary-dark hover:bg-accent-gold-dark transition shadow-sm">
        ...
        @if($arsip->file_arsip)
        <a href="{{ route('admin.arsip.download', $arsip) }}" class="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-lg bg-primary text-white hover:bg-primary-light transition shadow-sm">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <div class="lg:col-span-2 space-y-6">
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div class="p-5 grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    <p class="text-[0.7rem] font-semibold text-slate-400 uppercase tracking-wide">Jenis Dokumen</p>
    ...
    <div class="space-y-6">
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5 text-center">
            ...
            <p class="text-xs font-semibold text-slate-700 truncate">{{ $arsip->file_arsip ? basename($arsip->file_arsip) : 'Belum ada berkas' }}</p>
            <p class="text-[0.7rem] text-slate-400 mt-0.5">Diunggah {{ $arsip->created_at?->translatedFormat('d/m/Y') }}</p>
        </div>

        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
            <h3 class="font-semibold text-slate-800 text-sm mb-4">Log Aktivitas Terakhir</h3>
            <div class="flex gap-3">
                <div class="w-2 h-2 rounded-full bg-primary mt-1.5 shrink-0"></div>
                <div>
                    <p class="text-sm font-medium text-slate-700">Dokumen {{ $arsip->wasRecentlyCreated ?? false ? 'Dibuat' : 'Diperbarui' }}</p>
                    <p class="text-xs text-slate-400 mt-0.5">Oleh: {{ $arsip->user->name ?? '-' }} · {{ $arsip->updated_at?->diffForHumans() }}</p>
                </div>
            </div>
            @if($arsip->peminjaman->isNotEmpty())
            <a href="#" class="mt-4 block text-center text-xs font-semibold text-primary hover:underline">Lihat Riwayat Lengkap</a>
            @endif
        </div>
    </div>
</div>
