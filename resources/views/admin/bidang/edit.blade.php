...
@section('breadcrumb')
<a href="{{ route('admin.dashboard') }}" class="hover:text-primary">Dashboard</a> <span>/</span> <a href="{{ route('admin.bidang.index', ['tab' => 'bidang']) }}" class="hover:text-primary">Manajemen Bidang & User</a> <span>/</span> <span class="text-slate-700 font-medium">Edit Bidang</span>
@endsection
@section('content')
    ...
    <div class="lg:col-span-2 bg-white rounded-xl border border-slate-200 shadow-sm p-6">
        ...
        <form action="{{ route('admin.bidang.update', $bidang) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <input type="text" name="nama_bidang" value="{{ old('nama_bidang', $bidang->nama_bidang) }}" required class="w-full px-3.5 py-2.5 rounded-lg border-slate-300 text-sm focus:ring-primary focus:border-primary @error('nama_bidang') border-red-400 @enderror">
            ...
            <div class="mb-6">
                ...
                <textarea name="deskripsi" placeholder="Tuliskan deskripsi singkat mengenai fungsi bidang ini..." class="w-full px-3.5 py-2.5 rounded-lg border-slate-300 text-sm focus:ring-primary focus:border-primary" rows="4">{{ old('deskripsi', $bidang->deskripsi) }}</textarea>
            </div>
            ...
            <div class="flex items-center gap-3 pt-5 border-t border-slate-100">
                ...
                <a href="{{ route('admin.bidang.index', ['tab' => 'bidang']) }}" class="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-semibold rounded-lg border border-slate-300 text-slate-600 hover:bg-slate-50 transition">
