...
@section('breadcrumb')
<a href="{{ route('dashboard') }}" class="hover:text-primary">Manajemen</a> <span>/</span> <a href="{{ route('bidang.index') }}" class="hover:text-primary">Daftar Bidang</a> <span>/</span> <span class="text-slate-700 font-medium">Tambah Bidang</span>
@endsection
@section('content')
...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden mb-6">
    ...
    <form action="{{ route('bidang.store') }}" method="POST" class="p-6">
        @csrf
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div>
                ...
                <input type="text" name="nama_bidang" value="{{ old('nama_bidang') }}" required placeholder="Contoh: Bidang Infrastruktur dan Kewilayahan" class="w-full rounded-lg border-slate-300 text-sm focus:ring-primary focus:border-primary @error('nama_bidang') border-red-400 @enderror">
            ...
            <div>
                ...
                <input type="text" name="kode_bidang" value="{{ old('kode_bidang') }}" required placeholder="Contoh: INF-002" class="w-full rounded-lg border-slate-300 text-sm focus:ring-primary focus:border-primary @error('kode_bidang') border-red-400 @enderror">
        ...
        <div class="flex items-center justify-end gap-3 mt-8 pt-5 border-t border-slate-100">
            <a href="{{ route('bidang.index') }}" class="px-4 py-2.5 text-sm font-semibold rounded-lg border border-slate-300 text-slate-600 hover:bg-slate-50 transition">Batal</a>
