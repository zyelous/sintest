...
<div class="flex items-start justify-between mb-6 flex-wrap gap-3">
    <div>
        <h1 class="text-2xl font-bold text-slate-800">Manajemen Arsip Bidang</h1>
        <p class="text-sm text-slate-500 mt-1">Kelola data arsip untuk {{ auth()->user()->bidang->nama_bidang ?? 'Bidang Anda' }}.</p>
    </div>
    <div class="flex items-center gap-2">
        ...
        <a href="{{ route('operator.arsip.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-lg bg-primary text-white hover:bg-primary-light transition shadow-sm">
...
<div id="filterPanel" class="hidden bg-white rounded-xl border border-slate-200 p-5 mb-6 shadow-sm">
    <div class="flex items-center justify-between mb-4">
        ...
        <a href="{{ route('operator.arsip.index') }}" class="text-xs font-semibold text-primary hover:underline">Reset Filter</a>
    </div>
    <form method="GET" action="{{ route('operator.arsip.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="sm:col-span-2">
            <label class="block text-xs font-semibold text-slate-500 mb-1.5">Cari</label>
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Kode, no. berkas, uraian..." class="w-full px-3.5 py-2.5 rounded-lg border-slate-300 text-sm focus:ring-primary focus:border-primary">
        </div>
        <div>
            <label class="block text-xs font-semibold text-slate-500 mb-1.5">Kode Klasifikasi</label>
            <input type="text" name="kode_klasifikasi" value="{{ request('kode_klasifikasi') }}" placeholder="Contoh: 005/TU" class="w-full px-3.5 py-2.5 rounded-lg border-slate-300 text-sm focus:ring-primary focus:border-primary">
        </div>
        <div>
            <label class="block text-xs font-semibold text-slate-500 mb-1.5">Status Retensi</label>
            <select name="status_retensi" class="w-full px-3.5 py-2.5 rounded-lg border-slate-300 text-sm focus:ring-primary focus:border-primary">
        ...
        <div>
            ...
            <select name="klasifikasi_keamanan" class="w-full px-3.5 py-2.5 rounded-lg border-slate-300 text-sm focus:ring-primary focus:border-primary">
                <option value="">Semua</option>
        ...
        <div>
            ...
            <select name="status_arsip" class="w-full px-3.5 py-2.5 rounded-lg border-slate-300 text-sm focus:ring-primary focus:border-primary">
                <option value="">Semua</option>
        ...
        <div>
            ...
            <input type="text" name="no_rak" value="{{ request('no_rak') }}" placeholder="No. Rak" class="w-full px-3.5 py-2.5 rounded-lg border-slate-300 text-sm focus:ring-primary focus:border-primary">
        </div>
        <div>
            <label class="block text-xs font-semibold text-slate-500 mb-1.5">No. Boks</label>
            <input type="text" name="no_boks" value="{{ request('no_boks') }}" placeholder="No. Boks" class="w-full px-3.5 py-2.5 rounded-lg border-slate-300 text-sm focus:ring-primary focus:border-primary">
        </div>
        <div class="sm:col-span-2">
...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
    <div class="flex items-center justify-between px-5 py-4 border-b border-slate-100">
        <p class="text-sm text-slate-600"><span class="font-semibold text-slate-800">{{ number_format($arsipList->total()) }}</span> Total Arsip</p>
    </div>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-primary text-white text-xs uppercase tracking-wide">
                    ...
                    <th class="px-5 py-3 text-left font-semibold">Judul & Detail</th>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50">
                    ...
                    <td class="px-5 py-3.5 whitespace-nowrap">
                        @php
                            $statusColor = $arsip->status_arsip === 'dipinjam'
                    ...
                    <td class="px-5 py-3.5">
                        <div class="flex items-center justify-end gap-1">
                            <a href="{{ route('operator.arsip.show', $arsip) }}" class="p-1.5 rounded-md text-slate-400 hover:bg-slate-100 hover:text-primary transition" title="Lihat"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></a>
                            <a href="{{ route('operator.arsip.edit', $arsip) }}" class="p-1.5 rounded-md text-slate-400 hover:bg-slate-100 hover:text-amber-600 transition" title="Edit"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.12 2.12 0 0 1 3 3L12 15l-4 1 1-4z"/></svg></a>
                            <button type="button" onclick="confirmDelete('{{ route('operator.arsip.destroy', $arsip) }}')" class="p-1.5 rounded-md text-slate-400 hover:bg-red-50 hover:text-red-600 transition" title="Hapus"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button>
                        </div>
                    </td>
                </tr>
                @empty
                <tr><td colspan="6" class="px-5 py-10 text-center text-slate-400">Belum ada data arsip di bidang ini.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
