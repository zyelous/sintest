...
<div class="flex items-center justify-between mb-6">
    <div>
        ...
        <p class="text-sm text-slate-500 mt-0.5">Monitoring, persetujuan, dan pengembalian arsip fisik di bidang Anda.</p>
    </div>
    <a href="{{ route('operator.peminjaman.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-lg bg-primary text-white hover:bg-primary-light transition shadow-sm">
                ...
                <tr class="hover:bg-slate-50">
                    ...
                    <td class="px-5 py-3.5">
                        <div class="flex items-center justify-center gap-1.5">
                            <a href="{{ route('operator.peminjaman.show', $p) }}" title="Detail" class="w-8 h-8 flex items-center justify-center rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 transition">
                                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                            </a>

                            @if($p->status === 'menunggu_persetujuan')
                            <form method="POST" action="{{ route('operator.peminjaman.approve', $p) }}" class="inline">
                                @csrf @method('PUT')
                            ...
                            <form method="POST" action="{{ route('operator.peminjaman.reject', $p) }}" class="inline">
                                @csrf @method('PUT')
                            ...
                            <a href="{{ route('operator.peminjaman.edit', $p) }}" title="Edit" class="w-8 h-8 flex items-center justify-center rounded-lg bg-amber-50 text-amber-600 hover:bg-amber-100 transition">
                                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                            </a>
                            @endif

                            <button type="button" title="Hapus" onclick="confirmDelete('{{ route('operator.peminjaman.destroy', $p) }}')" class="w-8 h-8 flex items-center justify-center rounded-lg bg-rose-50 text-rose-500 hover:bg-rose-100 transition">
