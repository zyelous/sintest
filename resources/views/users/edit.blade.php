...
@section('breadcrumb')
    ...
    <span class="text-slate-300">/</span>
    <span class="text-slate-800 font-semibold">Edit User</span>
@endsection
...
@section('content')
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <h2 class="text-base font-semibold text-slate-700">Formulir Edit User</h2>
        </div>
            ...
            <form action="{{ route('users.update', $user->id) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400 mt-1">Kosongkan jika tidak diubah</p>
                        @error('password')
                            <p class="text-xs font-medium text-red-500 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    {{-- Bidang --}}
                    <div id="bidang-wrapper" class="{{ old('role', $user->role) === 'operator' ? '' : 'hidden' }}">
                        ...
                        <select id="bidang_id"
                            ...
                            <option value="" disabled {{ old('bidang_id', $user->bidang_id) ? '' : 'selected' }}>Pilih Bidang</option>
                    ...
                    <div>
                        ...
                        <select id="is_active"
                                ...
                                required>
                            <option value="1" {{ old('is_active', (string) $user->is_active) === '1' ? 'selected' : '' }}>Ya</option>
                            <option value="0" {{ old('is_active', (string) $user->is_active) === '0' ? 'selected' : '' }}>Tidak</option>
                        </select>
                        @error('is_active')
                            <p class="text-xs font-medium text-red-500 mt-1">{{ $message }}</p>
                        @enderror
